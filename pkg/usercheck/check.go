...
func (c *Check) Run() check.Result {
	...
	if c.UID != "" && u.Uid != c.UID {
		result.Status = check.StatusFail
		result.Details = append(result.Details, fmt.Sprintf("uid mismatch: expected %s, got %s", c.UID, u.Uid))
		result.Err = fmt.Errorf("uid mismatch")
		return result
	}

	// Check GID if specified
	if c.GID != "" && u.Gid != c.GID {
		result.Status = check.StatusFail
		result.Details = append(result.Details, fmt.Sprintf("gid mismatch: expected %s, got %s", c.GID, u.Gid))
		result.Err = fmt.Errorf("gid mismatch")
		return result
	}
	...
	if c.Home != "" && u.HomeDir != c.Home {
		result.Status = check.StatusFail
		result.Details = append(result.Details, fmt.Sprintf("home mismatch: expected %s, got %s", c.Home, u.HomeDir))
		result.Err = fmt.Errorf("home mismatch")
		return result
	}
